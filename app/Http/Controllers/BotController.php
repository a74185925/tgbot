    ...
    public function index(Request $request)
	{
        ...
        if ($data) {
            ...
            try {
                ...
                Storage::prepend('logs/log_customers.log', print_r($error_time.' ------- '.$th->getMessage(), true));
            }
            ...
            if ($chat_id && $text ) {
                switch ($text) {
                    case ($text == "/start" || $text == "ГЛАВНАЯ"):
                        ...
                        $this_massage = $bot_messages->find(2);
                        $the_message =  ( isset($this_massage->text) ) ? $this_massage->text : "Добро пожаловать!";
                        $resp_args = [
                            'chat_id'      => $chat_id,
                            'parse_mode'   => 'HTML',
                            'text'         => $the_message,
                        ...
                        break;
                    case "ПОМОЩЬ":
                        $this_massage = $bot_messages->find(1);
                        $the_message =  ( isset($this_massage->text) ) ? $this_massage->text : "Сообщение раздела помощь!";
                        $resp_args = [
                            'chat_id' => $chat_id,
                            'parse_mode' => 'HTML',
                            'text'=> $the_message,
                ...
                if (count($cq_data_arr) > 1) {
                    ...
                    try {
                        if ($cq_data_arr[0]== 'city') {
                            ...
                            // Выводит список районов или продуктов если нет районов!
                            ...
                            if ($districts->count()>0) {
                                ...
                                $this_massage = $bot_messages->find(3);
                                $cq_final_text = $this_massage->text.$this_city->name.$this_massage->text2;
                                ...
                                $this_massage = $bot_messages->find(4);
                                $cq_final_text = $this_massage->text.$this_city->name.$this_massage->text2;
                            }
                            ...
                            $cq_final_keyboard = $this->getInlineKeyBoard($products_list);
                            $this_massage = $bot_messages->find(5);
                            $cq_final_text = $this_massage->text.$this_city->name.$this_massage->text2.$this_distr->name.$this_massage->text3;
                            ...
                            $this_massage = $bot_messages->find(6);
                            $rtext = $this_massage->text.$this_city->name;
                            if ($this_distr) {
                                $rtext .= $this_massage->text2.$this_distr->name;
                            }
                            $rtext .= $this_massage->text3.$this_product->name.$this_massage->text4.$this_product->price.$this_massage->text5;
                            if ($this_product->description) {
                                $rtext .= "\n".$this_product->description."\n";
                            }
                            $rtext .= $this_massage->text6;
                            ...
                            $cq_final_text = $bot_messages->find(2)->text;
                            ...
                            if ($exchang_rates->failed()) {
                                ...
                                $this_massage = $bot_messages->find(12);
                                $cq_final_text = $this_massage->text;
                                ...
                                if ($invoice_resp->failed()) {
                                    ...
                                    $this_massage = $bot_messages->find(13);
                                    $cq_final_text = $this_massage->text;
                                    $cq_final_keyboard = $this->getInlineKeyBoard([[['text' => "C начала", 'callback_data' => 'home_0_0']]]);
                                } else {
                                    ...
                                    $this_massage = $bot_messages->find(7);
                                    $rtext = $this_massage->text.$order->id.$this_massage->text2.$this_city->name;
                                    if ($this_distr) {
                                        $rtext .= $this_massage->text3.$this_distr->name;
                                    }
                                    $rtext .= $this_massage->text4.$this_product->name.$this_massage->text5.$this_product->price.$this_massage->text6;
                                    $rtext .= $this_massage->text7."<b>".$crypto_price_text."</b> ".$pay_method_name.$this_massage->text8."<b>".$pay_address."</b>";
                            ...
                            if ($this_order->status == 'completed') {
                                if ($this_order->loot) {
                                    ...
                                    $this_massage = $bot_messages->find(11);
                                    $cq_final_text = $this_massage->text.$this_order->id.$this_massage->text2;
                                    $cq_final_text .= $loot_text;
                                    $final_photo = config('app.url').'/'.$this_loot->img;

                                } else {
                                    $this_massage = $bot_messages->find(14);
                                    $cq_final_text = $this_massage->text.$this_order->id.$this_massage->text1;
                                }
                                ...
                                if ($invoice_status_resp->failed()) {
                                    ...
                                    if ($invoice_status_obj->status == 'created') {
                                        ...
                                        if ($customer_loot) {
                                            ...
                                            $customer_loot->save();
                                            // Для юзера инфо по посылке
                                            $loot_text = $customer_loot->text;
                                            $this_massage = $bot_messages->find(11);
                                            $cq_final_text = $this_massage->text.$this_order->id.$this_massage->text2;
                                            $cq_final_text .= $loot_text;
                                            $final_photo = config('app.url').'/'.$customer_loot->img;
                                        } else {
                                            $this_massage = $bot_messages->find(15);
                                            $cq_final_text = $this_massage->text.$this_order->id.$this_massage->text2;
                                        }

                                    } elseif ($invoice_status_obj->status == 'partpaid') {
                                        $this_massage = $bot_messages->find(10);
                                        $cq_final_text = $this_massage->text;
                                    } else {
                                        $this_massage = $bot_messages->find(16);
                                        $cq_final_text = $this_massage->text;
                                        $cq_final_keyboard = $this->getInlineKeyBoard([[
                        ...
                        if (isset($cq_final_text) && isset($cq_final_keyboard)) {
                            ...
                            if (isset($final_photo)) {
                                $final_photo_args = [
                                    'chat_id' => $cq_chat_id,
                                    'photo' => $final_photo
                                ];
                                $final_photo_resp = $this->send_request('sendPhoto', $final_photo_args);
                            }
                        ...
                        $this_massage = $bot_messages->find(17);
                        $cq_final_text = $this_massage->text;
                ...
                Storage::prepend('log/bad-bot-request.log', '***********************'.$chat_id.'***********************');
                Storage::prepend('log/bad-bot-request.log', print_r($data, true));
                Storage::prepend('log/bad-bot-request.log', '***********************'.$chat_id.'***********************');                
            }
        }
